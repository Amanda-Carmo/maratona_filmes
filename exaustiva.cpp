...
int scorePlaylist(vector<Movie>& playlist) {
    ...
    for (auto& movie : playlist) {
        score += movie.duration;
    }
...
void findMovies(vector<Movie> movies, vector<int> max_movies, vector<Movie> playlist,
    ...
    for (int i = 0; i < movies.size(); i++) {
        // Chamada recursiva na qual o filme não é assistido
        // Remover o filme do vetor de filmes
        Movie current_movie
            = *movies_temp.erase(movies_temp.begin()); // ponteiro para o filme romovido

        // findMovies(movies_temp, max_movies, playlist, best_score, best_playlist);
        bool adicionado = false;

        if (canWatch(current_movie, playlist, max_movies)) {
            if (current_movie.end_time > current_movie.start_time) {
                max_movies[current_movie.category - 1]
                    -= 1; // Diminuir a quantidade máxima de filmes da categoria do filme assistido

                // Adicionar na playlist
                playlist.push_back(current_movie);
                adicionado = true;
            }
        }
        findMovies(movies_temp, max_movies, playlist, best_score, best_playlist);
        ...
        movies_temp.push_back(current_movie); // Adicionar o filme removido novamente

        if (adicionado) {
            max_movies[current_movie.category - 1]
                += 1; // Aumentar a quantidade máxima de filmes da categoria do filme assistido
            playlist.pop_back(); // Remover o filme da playlist
        }
    }
}
...
int main() {
    ...
    // for (auto& movie : best_playlist) {
    //     cout << movie.id << " " << movie.start_time << " " << movie.end_time << " "
    //          << movie.duration << " " << movie.category << "\n";
    // }

    // Imprime quantidade de filmes assistidos
    cout << best_playlist.size() << "\n";

    // Imprimir melhor score
    cout << best_score << "\n";
